    ...
    await redisSub.subscribe("sendEmail-task", async (message) => {
        ...
        try {
            ...
            cron.schedule(cronTime, async () => {
                console.log(`[Worker] Running scheduled task for ${emailTo} at ${new Date().toISOString()}`);
                const { totalSpace, freeSpace, usedSpace, path } = await getDiskUsage();
                new DiskSpaceNotification(emailTo, containerId, totalSpace, usedSpace, freeSpace, path).sendEmail(finalSubject, "container-usage");
            });
            console.log(`[Worker] Cron job scheduled for ${emailTo} with time: ${cronTime}`);
        }
        catch (e) {
