        ...
        switch (jobName) {
            case "create-folder":
                try {
                    ...
                    throw new Error("Sorry unable to create folder with path " + pathName);
                }
                finally {
                    break;
                }
            case "create-file":
                try {
                    ...
                    throw new Error("Sorry unable to create file with path " + pathName);
                }
                finally {
                    break;
                }
            case "delete-file":
                try {
                    ...
                    throw new Error("Sorry unable to delete file with path " + pathName);
                }
                finally {
                    break;
                }
            case "delete-folder":
                try {
                    ...
                    throw new Error("Sorry unable to delete folder with path " + pathName);
                }
                finally {
                    break;
                }
...
const readOpsWorker = async () => {
    const readWorker = new Worker("read-ops", async (job) => {
        const jobName = job.name;
        const { pathName } = job.data;
        console.log('JOB DATA IS:', job.data);
        switch (jobName) {
            case "listing-directory":
                try {
                    if (!pathName)
                        return new Error("Bad Request : pathName not given");
                    readController.listingDirectory(pathName);
                }
                catch {
                    throw new Error("Sorry unable to list folder with path " + pathName);
                }
                finally {
                    break;
                }
